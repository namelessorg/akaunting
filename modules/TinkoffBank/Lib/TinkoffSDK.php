...
namespace Modules\TinkoffBank\Lib;
...
class TinkoffSDK
{
    ...
    private function _sendRequest($api_url, $args)
    {
        ...
        if ($curl = curl_init()) {
            ...
            $out = curl_exec($curl);
            $this->response = $out;
            $json = json_decode($out, false , 512, JSON_THROW_ON_ERROR | JSON_UNESCAPED_UNICODE);

            if ($json) {
                if (($json->ErrorCode ?? null) !== "0") {
                    $this->error = $json->Details ?? null;
                } else {
        ...
        throw new \DomainException('Can not create connection to ' . $api_url . ' with args ' . $args, 404);
    }
}
